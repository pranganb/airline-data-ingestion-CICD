...
def main(env, bq_project, bq_dataset, transformed_table, route_insights_table, origin_insights_table):
    try:
        ...
        input_path = f"gs://mybucket-airflow/airline_data/source-{env}"
        logger.info(f"Input path resolved: {input_path}")


        # Read the data from GCS
        data = spark.read.csv(input_path, header = True, inferSchema = True)
        logger.info(f"Data read from CSV")
        ...
        logger.info(f"Writing transformed data to BigQeury table: {bq_project}:{bq_dataset}.{transformed_table}")
        ...
        logger.info(f"Writing route insights")
